...
class Strategy33(StrategyBase):
    ...
    def __init__(self,stock,stockqtty,asset1,qtty1,asset2,qtty2,fee):

        StrategyBase.__init__(self,"collar",stock,stockqtty=stockqtty,asset1=asset1,qtty1=qtty1,asset2=asset2,qtty2=qtty2)
        ...
        #out1 from split 1: output element values = asset1.strike - asset1.price - stock.price + asset2.price
        vout1 = asset1.strike - asset1.price - stock.price + asset2.price
        out1 = np.full(split1.shape,vout1)
        #out3 from split 3: output element values = asset2.price + asset2.strike - stock.price - asset1.price
        vout3 = asset2.price + asset2.strike - stock.price - asset1.price
        out3 = np.full(split3.shape,vout3)


        #3. use split results to create function valued arrays

        #out 2 from split 2: output elements func = self.priceatmaturity[i] + asset2.price - asset1.price - stock.price
        vout2 = asset2.price - asset1.price - stock.price
