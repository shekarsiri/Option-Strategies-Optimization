...
class Strategy42(StrategyBase):
    ...
    def __init__(self,stock,asset1,qtty1,asset2,qtty2,fee):
        ...
        StrategyBase.__init__(self,"long put spread",stock,asset1=asset1,qtty1=qtty1,asset2=asset2,qtty2=qtty2)
        ...
        #out1 from split 1: output element values = asset2.strike + asset1.price - asset2.price - asset1.strike
        vout1 = asset2.strike + asset1.price - asset2.price - asset1.strike
        out1 = np.full(split1.shape,vout1)
        #out3 from split 3: output element values = asset1.price - asset2.price
        vout3 = asset1.price - asset2.price
        out3 = np.full(split3.shape,vout3)


        #3. use split results to create function valued arrays

        #out 2 from split 2: output elements func = -asset2.price + asset1.price - self.priceatmaturity[i] + asset2.strike
